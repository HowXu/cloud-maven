...
import { FolderArchive, Trash2, X } from "lucide-vue-next";
...
const remove = async () => {
  ...
  try {
    ...
    createToast(props.entry.type === "DIRECTORY" ? "Artifact directory deleted" : "Artifact deleted", {
      type: "success",
  ...
  <div>
    ...
    <VueFinalModal v-model="show" class="flex items-center justify-center px-4">
      <div class="modal-panel">
        ...
        <div v-if="entry.type === 'DIRECTORY'" class="mb-4 flex items-start gap-3 rounded-lg bg-amber-50 p-3 text-amber-800 dark:bg-amber-900/30 dark:text-amber-200">
          <FolderArchive class="mt-0.5 h-5 w-5 shrink-0" />
          <p class="text-sm">
            This removes the entire artifact directory and <strong>all its files</strong> at
            <code>{{ entryPath }}</code>. This action cannot be undone.
          </p>
        </div>
