...
import { getCacheControl, getContentType, isChecksumFile, isMetadataFile } from '../../src/shared'
...
describe('Maven MIME and cache helpers', () => {
  ...
  it('uses immutable caching for artifacts and no-cache for metadata', () => {
    expect(getCacheControl('com/example/demo/1.0.0/demo-1.0.0.jar')).toBe('public, max-age=31536000, immutable')
    expect(getCacheControl('com/example/demo/1.0.0/demo-1.0.0.pom')).toBe('public, max-age=31536000, immutable')
    expect(getCacheControl('com/example/demo/maven-metadata.xml')).toBe('no-cache')
  })

  it('classifies checksum and metadata artifacts', () => {
