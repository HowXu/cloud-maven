...
- **参考**: `agents/Client.md`（工作手册）、`Client.md`（设计方案）
...
- **参考**: `agents/Worker.md`（工作手册）、`Worker.md`（设计方案）
...
- Agent 只能读取全局文件（`AGENTS.md`、`USAGE.md`），但不能修改其他角色的专属文件
...
参见各 `agents/xxx.md` 文件中指向自己的「待处理」交接记录。
