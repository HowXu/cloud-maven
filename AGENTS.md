...
Cloud-Maven 是基于 Cloudflare Workers、R2、KV 构建的轻量 Maven 仓库系统，支持一键部署。由 Vue 3 前端管理界面 + Cloudflare Worker 后端 API 组成，不创建 `releases`/`snapshots` 默认仓库，Maven 路径直接映射到 R2 Key。
...
| 文件 | 用途 | 谁维护 |
|------|------|--------|
| `AGENTS.md` | 本文档，Agent 操作总纲 | 全员参考，结构角色维护 |
| `PROJECT.md` | 项目目标、总体约束、环境声明 | 结构角色 |
| `STRUCT.md` | 结构控制、Agent 协作边界、需求交接记录 | 结构角色 |
| `API.md` | 前后端共用 API 契约 | 前端提出需求，后端确认实现 |
| `Client.md` | 前端设计方案（详细） | 前端角色 |
| `Worker.md` | 后端设计方案（详细） | 后端角色 |
| `USAGE.md` | 部署到 Cloudflare 的操作指南 | README文书角色 |
| `Failed.md` | 测试失败记录与分析 | 测试角色 |
| `workspace.config.json` | 机器可读的项目清单与任务编排 | 结构角色 |
| `scripts/workspace.mjs` | 跨端任务执行器 | 结构角色 |
| `agents/Client.md` | 前端 Agent 工作手册 | 前端角色 |
| `agents/Worker.md` | 后端 Agent 工作手册 | 后端角色 |
...
- **参考**: `agents/Client.md`（工作手册）、`Client.md`（设计方案）、`API.md`（接口契约）
- **禁止**: 修改 `maven-worker/` 任何文件；绕过 `API.md` 约定临时接口；执行 npm 命令

### 2. 后端角色 (Worker Agent)

- **工作目录**: `maven-worker/` — 仅对该目录有读写权限
- **职责**: Cloudflare Worker 后端 API 开发，包括路由、存储、鉴权、Maven 协议
- **参考**: `agents/Worker.md`（工作手册）、`Worker.md`（设计方案）、`API.md`（接口契约）
- **禁止**: 修改 `maven-client/` 任何文件；改变 API 响应结构而不更新 `API.md`；执行 npm 命令

### 3. 测试角色 (Test Agent)

- **工作目录**: `maven-client/src/**/*.test.ts` 和 `maven-worker/test/**/*.test.ts`
- **职责**: 编写和维护单元测试、集成测试；运行测试并记录失败；修复测试基础设施问题（mock、pool 兼容性等）
- **参考**: `agents/Test.md`（工作手册）、`Failed.md`（失败记录）
...
- **禁止**: 直接修改业务代码；执行 npm 命令。审计结果写入 `agents/Secur.md` 和 `STRUCT.md` 需求交接记录
...
- Agent 只能读取全局文件（`PROJECT.md`、`STRUCT.md`、`API.md`、`AGENTS.md`），但不能修改其他角色的专属文件
- 前端/后端互相需要对方配合时，**不得直接修改对方代码**，必须在 `STRUCT.md` 写入需求交接记录
- 测试角色发现业务 bug 时，在 `Failed.md` 记录，并在 `STRUCT.md` 写入交接记录指向对应角色

## 交接方式

跨角色协作通过 `STRUCT.md` 的「需求交接记录」章节进行。

### 写入交接记录

当单端 Agent 需要另一端配合时，在 `STRUCT.md` 末尾按以下格式新增条目：

```markdown
### YYYY-MM-DD - <来源角色> -> <目标角色>

状态：待处理
来源：Client.md | Worker.md | API.md | agents/xxx.md | 具体文件路径
需求：
- 需要对方完成的事情。
验收：
- 如何判断这项配合已经完成。
...
- 目标角色 Agent 启动时，应首先阅读 `STRUCT.md` 中指向自己的「待处理」交接记录
...
| 测试 | `*.test.ts`、`vitest.config.ts`、`Failed.md` | 业务源码 |
| 安全审计 | `agents/Secur.md`、`STRUCT.md`（交接记录） | 业务源码 |
| README文书 | 根目录 `.md`、`maven-worker/README.md` | 业务源码、`API.md` 接口定义 |

### 根目录文件修改权限

| 文件 | 前端 | 后端 | 测试 | 安全审计 | README |
|------|------|------|------|----------|--------|
| `PROJECT.md` | 只读 | 只读 | 只读 | 只读 | 可写 |
| `STRUCT.md` | 追加交接 | 追加交接 | 追加交接 | 追加交接 | 只读 |
| `API.md` | 可写（需求） | 可写（确认） | 只读 | 只读 | 只读 |
| `Client.md` | 可写 | 只读 | 只读 | 只读 | 只读 |
| `Worker.md` | 只读 | 可写 | 只读 | 只读 | 只读 |
| `USAGE.md` | 只读 | 只读 | 只读 | 只读 | 可写 |
| `Failed.md` | 只读 | 只读 | 可写 | 只读 | 只读 |
