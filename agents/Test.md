...
- `../Failed.md` — 测试失败记录（核心输出文件）
- `../STRUCT.md` — 需求交接记录（发现业务 bug 时通知对应角色）
- `../AGENTS.md` — 角色边界定义
...
1. **运行测试**: 先查阅 `Failed.md` 了解已知问题，运行测试收集结果
2. **分析失败**: 区分「测试代码问题」和「业务代码问题」
3. **测试代码问题**（mock 不全、配置错误等）：直接修复
4. **业务代码问题**（逻辑 bug、接口不符等）：
   - 写入 `Failed.md` 记录
   - 写入 `STRUCT.md` 交接记录指向对应角色
5. **新测试编写**: 按需求编写覆盖正常路径、边界条件和错误路径的测试

## 注意事项

- 不执行 npm 命令安装新依赖
- 测试代码只能修改 `*.test.ts`、`vitest.config.ts`、`Failed.md`
- 发现业务 bug 不直接修改业务源码，通过 `STRUCT.md` 交接
- 后端集成测试需要 Cloudflare Workers pool，不能简单用 `--pool=forks` 替代
